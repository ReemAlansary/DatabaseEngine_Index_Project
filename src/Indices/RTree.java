...
public class RTree implements Serializable {
    ...
    public RTree(String tableName, String column) {
...
// ---------------------------------------------------------------------------------------------------------------------
    protected void updateUpper(Comparable oldKey, Comparable newKey, String pathToNode) {
        ...
        for (NodeEntry e : nextNode.entries) {
            if (((CPolygon) (e.key)).equals(oldKey)) {
                e.key = newKey;
...
// ---------------------------------------------------------------------------------------------------------------------
    public Pointer find(Comparable key) {
        ...
        for (int i = 0; i < r.pointers.size(); i++) {
            ...
            if (k.key.compareTo(key) == 0) return r.pointers.get(i);
    ...
    private String printTree(String path) {
        ...
        if (n instanceof Leaf) {
            ...
            for (int i = 0; i < l.max; i++) {
                if (i < l.pointers.size()) {
                    res += l.pointers.get(i).key.toString() + ",";
                    if (l.pointers.get(i) instanceof TuplePointer)
                        res += l.pointers.get(i).pagePath + "," + ((TuplePointer) l.pointers.get(i)).idx;
                    else {
                        res += "(";
                        OverFlowPage ovp = (OverFlowPage) DBApp.readFile(l.pointers.get(i).getPagePath());
                        for (int j = 0; j < ovp.pointers.size(); j++) {
                            res += ovp.pointers.get(j).pagePath + "-" + ovp.pointers.get(j).idx;
                            if (j < ovp.pointers.size() - 1) res += ",";
                        }
                        res += ")";
                    }
                } else res += " ";
...
// ---------------------------------------------------------------------------------------------------------------------
    ...
    public Pointer findCoordinates(Comparable key) {
        Leaf r = (Leaf) DBApp.readFile(findLeaf(rootPath, key, null));
        for (int i = 0; i < r.pointers.size(); i++) {
            Pointer k = r.pointers.get(i);
            if (((CPolygon) k.key).equals((CPolygon) key))
                if (((CPolygon) k.key).equal((CPolygon) key))
